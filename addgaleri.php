...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($type2, $tipe_diizinkan)) {
            ...
            // Proses upload file
            if (move_uploaded_file($tmp_name, 'img/galeri/' . $newname)) {
                $foto = $newname; // Set foto dengan nama baru

                // Simpan data ke database
                $insert_query = "INSERT INTO galeri (foto) VALUES (?)";
                $insert_stmt = $conn->prepare($insert_query);
                $insert_stmt->bind_param("s", $foto);
        ...
        echo '<script>alert("Silakan pilih foto untuk diupload.")</script>';
    }
}
?>
