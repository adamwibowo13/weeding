...
// Ambil ID dari URL untuk mengedit data
$id = $_GET['id'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $foto = $data['foto']; // Default ke foto lama
    ...
    $update_query = "UPDATE story SET title = ?, tanggal = ?, keterangan = ?, foto = ? WHERE id = ?";
    $update_stmt = $conn->prepare($update_query);
    $update_stmt->bind_param("ssssi", $title, $tanggal, $keterangan, $foto, $id);
    
    if ($update_stmt->execute()) {
        echo '<script>alert("Data berhasil diupdate!"); window.location="story.php";</script>';
    } else {
              ...
              <!-- HTML Form untuk Mengedit Data -->
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="date" class="form-control" name="tanggal" value="<?php echo htmlspecialchars($data['tanggal']); ?>" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="keterangan" class="col-sm-2 col-form-label">Keterangan</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" name="keterangan" value="<?php echo htmlspecialchars($data['keterangan']); ?>">
                        </div>
                    </div>
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            ...
                            <?php if (!empty($data['foto'])): ?>
                                <img src="img/foto-story/<?php echo htmlspecialchars($data['foto']); ?>" alt="Foto Story" width="100" class="mt-2">
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="row mb-3">
